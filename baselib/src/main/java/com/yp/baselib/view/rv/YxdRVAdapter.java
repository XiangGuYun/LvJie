 ...
 * @date 2016/7/21.
 */
public abstract class YxdRVAdapter<T> extends RecyclerView.Adapter<YxdRVHolder> implements YxdDataHelper<T> {
    ...
    public YxdRVAdapter(Context context, List<T> list, int... layoutIds) {
    ...
    @Override
    public YxdRVHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        YxdRVHolder viewHolder = (YxdRVHolder) view.getTag();
        if (viewHolder == null || viewHolder.getLayoutId() != layoutId) {
            viewHolder = new YxdRVHolder(mContext, layoutId, view);
            return viewHolder;
        }
        return viewHolder;
    }
    ...
    @Override
    public void onBindViewHolder(YxdRVHolder holder, int position) {
    ...
    protected abstract void onBindData(YxdRVHolder viewHolder, int position, T item);
