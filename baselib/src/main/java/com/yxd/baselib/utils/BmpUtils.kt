...
object BmpUtils {
     ...
     * @param iv ImageView
     * @param imgUrl String?
     * @param placeholderDrawable Triple<Int, Int, Int>?
     * @param placeholderRes Int
     * @param override Pair<Int, Int> 用给定的值覆盖目标的宽度和高度。这对于缩略图很有用，
     *        并且应该只在需要特定图像大小的其他情况下使用，单位是像素。类似于根据目标宽高来计算采样率。
    ...
    fun show(
        ...
        Glide.with(iv.context).load(imgUrl).apply(req)
            ...
            .addListener(object : RequestListener<Drawable> {
                ...
                override fun onLoadFailed(
                    ...
                    LogUtils.d("YXD_Glide", "==== " + e?.message + "===== " + e?.localizedMessage)
                    return false
                }
    ...
    fun show(
        ...
        req.priority(priority)
        Glide.with(BaseApplication.getInstance().context).load(imgUrl).apply(req)
            .transition(DrawableTransitionOptions.withCrossFade())
            .addListener(object : RequestListener<Drawable> {
                override fun onResourceReady(
                    resource: Drawable?, model: Any?, target: Target<Drawable>?,
    ...
    fun url2Bmp(ctx: Context, imgUrl: String, getBitmap: (bmp: Bitmap) -> Unit) {
        Glide.with(ctx).load(imgUrl).apply(
            RequestOptions()
//                .override(320, 240).
    ...
    fun getVideoInfo(path: String): Triple<String, String, String> {
        ...
        media.setDataSource(path)
        val duration =
            media.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_DURATION)
        val width =
            media.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH)
        val height =
            media.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT)
     ...
     *
     * @param origin 原图
     * @param value  旋转角度，可正可负
     * @return 旋转后的图片
     */
    fun rotateBitmap(origin: Bitmap, value: Float): Bitmap {
        val width = origin.width
        val height = origin.height
        val matrix = Matrix()
