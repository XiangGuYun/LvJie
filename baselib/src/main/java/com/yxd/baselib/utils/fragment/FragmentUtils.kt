package com.yxd.baselib.utils.fragment
...
class FragmentUtils<T : Fragment> {
    ...
    constructor(a: FragmentActivity, list: List<T>, contentId: Int, firstIndex: Int = 0) {
        act = a
        manager = a.supportFragmentManager
        this.contentId = contentId
        val transaction = manager!!.beginTransaction()
        ...
        if(list.size > firstIndex){
            transaction.replace(contentId, list[firstIndex])
        }
        transaction.commitAllowingStateLoss()
    }

    constructor(a: FragmentActivity, list: ArrayList<T>, contentId: Int, firstIndex: Int = 0) {
        act = a
        manager = a.supportFragmentManager
        this.contentId = contentId
        val transaction = manager!!.beginTransaction()
    ...
    fun switchFrag(index: Int): Boolean {
        val targetFragment = fragments[index]
        //fragments.remove(targetFragment)
        val transaction = manager!!.beginTransaction()
        if (!targetFragment.isAdded) {    // 先判断是否被add过
