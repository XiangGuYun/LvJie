...
import com.yxd.baselib.listener.OnSeekBarChange
import com.yxd.baselib.utils.DensityUtils.Companion.dp2px
...
//    }
    ...
    fun View.view2Bitmap(callback: (Bitmap) -> Unit) {
        post {
            ...
            val bitmap =
                Bitmap.createBitmap(getDrawingCache()) //创建一个DrawingCache的拷贝，因为DrawingCache得到的位图在禁用后会被回收
     ...
     * @param et1 EditText
     * @param et2 EditText
     * @param getInput Function2<String, String, Boolean>
     * @param getResult Function1<Boolean, Unit>
     */
    @SuppressLint("CheckResult")
    fun formVerify(
        et1: EditText,
        et2: EditText,
        getInput: (String, String) -> Boolean,
        getResult: (Boolean) -> Unit
    ) {
        Observable.combineLatest(
            RxTextView.textChanges(et1).map {
                it.toString()
            },
            RxTextView.textChanges(et2).map {
                it.toString()
            },
            { s1, s2 ->
                getInput.invoke(s1, s2)
            })
            .subscribe { isOK ->
                getResult.invoke(isOK)
            }
    }

    /**
     * 表单验证，可验证更多的表单
     * @param etArray Array<EditText>
     * @param getInput Function1<Array<out Any>, Boolean>
     * @param getResult Function1<Boolean, Unit>
     */
    @SuppressLint("CheckResult")
    fun formVerify(
        etArray: Array<EditText>,
        getInput: (Array<out Any>) -> Boolean,
        getResult: (Boolean) -> Unit
    ) {
        Observable.combineLatest(
            etArray.map {
                RxTextView.textChanges(it).map {
                    it.toString()
                }
            }
        ) {
