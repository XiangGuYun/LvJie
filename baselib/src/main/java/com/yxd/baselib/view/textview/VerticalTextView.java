package com.yxd.baselib.view.textview;
...
public class VerticalTextView extends AppCompatTextView {
    ...
    public VerticalTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //　默认为4dp
        mCharSpacing = DensityUtils.Companion.dp2px( 4);
        mErrorDistance = DensityUtils.Companion.dp2px( 8);

        mShadowPaint = new Paint();
        mShadowPaint.setColor(Color.parseColor("#88EDECEE"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsDrawShadow) {
            mShadowPath.reset();
            mShadowPath.moveTo(getWidth(), getHeight() / 8);
            mShadowPath.quadTo(mShadowOffset/2, getHeight() / 2, getWidth(), getHeight() / 8 * 7);
